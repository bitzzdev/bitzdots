...
bitzdots is a collection of Hyprland dotfiles focused on low-end performance, automatic matugen theming, and a clean workflow. It includes configurations for waybar, rofi, swaync, kitty, fish, and **25 matugen templates** covering 20+ components.
...
- **Safe theming** — Backup/restore on every matugen image
...
- Qt6 config is auto-generated by matugen at `~/.config/qt6ct/qt6ct.conf`
...
Make sure matugen is installed and templates are in `matugen/templates/`. Regenerate manually:
...
### Matugen takes too long or fails with "Not enough colors!"
...
matugen image image.jpg --config ~/.config/matugen/config.toml
```
...
Ensure matugen has generated `~/.config/rofi/theme-generated.rasi`:
...
The rofi power menu uses themed SVG icons at `~/.config/rofi/icons/*.svg`. They're generated by matugen templates (`lock.svg.j2`, `logout.svg.j2`, etc.). Regenerate the theme to create them.

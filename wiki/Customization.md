...
3. Add the target to `WALLUST_OUTPUTS` in `wallust-cache-daemon.sh` and `wallpaper-select.sh` (backup/restore lists)
4. Add a reload step to `reload-theme.sh` if the app needs restarting
5. Regenerate: `wallust run ~/.cache/current_wallpaper.png --config-dir ~/.config/wallust`
